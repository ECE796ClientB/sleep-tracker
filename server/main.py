...
def getExerciseHours(patientId, startTime, endTime):
    ...
    for obs in observations:
        ...
        # If it's a new date, append the value
        if obs.effectiveDateTime.as_json() != curDate:
            # Append value
            exerciseHours.append( obs.valueQuantity.value )
            curDate = obs.effectiveDateTime.as_json()

        # Else, add the value to the last index
        else:
            exerciseHours[len(exerciseHours)] += obs.valueQuantity.value
...
def getCaffeineIntake(patientId, startTime, endTime):
    ...
    for obs in observations:
        ...
        if obs.effectiveDateTime.as_json() != curDate:
            # Append value
            caffeineIntakes.append( obs.valueQuantity.value )
            curDate = obs.effectiveDateTime.as_json()

        # Else, add the value to the last index
        else:
            caffeineIntakes[len(caffeineIntakes)] += obs.valueQuantity.value
